...
router.post('/products', JwtUtil.checkToken, async function (req, res) {
  ...
  // Kiểm tra trùng tên với hình ảnh
  const isDuplicate = await ProductDAO.checkDuplicateNameAndImage(name, image);
  if (isDuplicate) {
    return res.json({ success: false, message: 'Name and image must be unique' });
  }
